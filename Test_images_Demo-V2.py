...
if __name__ == '__main__':
    ...
    get_file_info = re.compile("(\d{1,2})_(\d+)x(\d+)")                         # original: get_file_info = re.compile("(\w+\d{1,2})_(\d+)x(\d+)")
    for pred, imgname in zip(predictions[0], file_names):
        ...
        # cv2.imwrite(output_folder + name + '_woCB.png', res.astype(int))       # we comment this line
        if with_CB:
            res = addCB(res,cmap)
        cv2.imwrite(output_folder + name + '.png', res.astype(int))

        with open(output_folder + name + '_2048x1024_32b.bin', "wb") as f:       #changed for the required formats
